...
export class KangentHttpClient {
	...
	async createBoard(params: {
		...
		return this.request<{ id: string; url: string; board: unknown }>("POST", "/api/boards", params);
	}
	...
	async updateBoard(
		...
		params: {
			title?: string;
			// Pass `null` to clear the description; omit to leave unchanged.
			description?: string | null;
			by: string;
		},
	) {
		return this.request<{ board: unknown; version: number }>(
			"PATCH",
			`/api/boards/${boardId}`,
			params,
		);
	...
	async reorderColumns(boardId: string, params: { columnIds: string[]; by: string }) {
		return this.request<{ columns: unknown[]; version: number }>(
			"POST",
