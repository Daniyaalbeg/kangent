// SKILL.md served at /kangent.SKILL.md. All URLs inside are absolute so the
// file stays valid no matter where the agent cached it. The `baseUrl` is the
// origin the user hit (e.g. http://localhost:8787 in dev, https://kangent.app
// in prod) so the skill a user downloads always points back at the same host.
export function buildSkillMd(baseUrl: string): string {
	return `---
name: kangent
...
The public URL for this install: ${baseUrl}
...
\`\`\`
GET ${baseUrl}/api/boards/<boardId>/changes
...
\`\`\`
GET ${baseUrl}/api/boards/<boardId>/state
Authorization: Bearer <token>
\`\`\`
...
### Move a card
\`\`\`
POST ${baseUrl}/api/boards/<boardId>/cards/<cardId>/move
{ "toColumnId": "col_2", "position": 0, "by": "ai:claude" }
\`\`\`

### Delete a card
\`\`\`
DELETE ${baseUrl}/api/boards/<boardId>/cards/<cardId>?by=ai:claude
\`\`\`

### Columns
\`\`\`
POST   ${baseUrl}/api/boards/<boardId>/columns          { "title": "QA", "by": "ai:claude" }
PATCH  ${baseUrl}/api/boards/<boardId>/columns/<colId>  { "title": "Done", "by": "ai:claude" }
DELETE ${baseUrl}/api/boards/<boardId>/columns/<colId>?moveCardsTo=col_2&by=ai:claude
\`\`\`
...
\`\`\`
POST ${baseUrl}/api/boards/<boardId>/presence
